...
type AppRole = "initiator" | "approver" | "auditor" | "super_admin" | "system_admin" | "operation";
...
const roleConfig: Record<AppRole, { label: string; description: string; category: "bank" | "platform" }> = {
  ...
  super_admin: { label: "Super Admin", description: "Full platform control", category: "platform" },
  system_admin: { label: "System Admin", description: "Manages system configuration", category: "platform" },
  operation: { label: "Operations", description: "Handles daily operations", category: "platform" },
};

const bankRoles: AppRole[] = ["initiator", "approver", "auditor"];
const platformRoles: AppRole[] = ["super_admin", "system_admin", "operation"];
const rolesRequiring2FA: AppRole[] = ["approver", "auditor", "super_admin"];

/* -------------------------------- MOCK DATA ------------------------------- */

const MOCK_USERS: UserWithRoles[] = [
  {
    id: "1",
    full_name: "John Doe",
    email: "john@bank.ae",
...
export default function UserManagement() {
  const [users, setUsers] = useState<UserWithRoles[]>(MOCK_USERS);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedUser) {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.id === selectedUser.id
            ? { ...u, ...formData, roles: formData.roles }
            : u
        )
      );
    } else {
      ...
      const newUser: UserWithRoles = {
        ...
        ...formData,
        is_active: true,
        is_2fa_enabled: formData.roles.some(r => rolesRequiring2FA.includes(r)),
        created_at: new Date().toISOString(),
      };
  ...
  return (
    <DashboardLayout title="User Management" subtitle="Manage users, roles, and access permissions">
      <Header
        ...
        subtitle="Complete activity user for compliance"
      />
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard icon={Building2} label="Bank Users" value={users.filter((u) => u.roles.some((r) => bankRoles.includes(r))).length} />
    ...
    <div className="grid gap-0.5">
      <label htmlFor={role} className="text-sm font-semibold leading-none flex items-center gap-2 cursor-pointer">
        ...
        {rolesRequiring2FA.includes(role) && (
          <Badge variant="secondary" className="text-[9px] h-4">MFA REQUIRED</Badge>
        )}
      </label>
