...
const adminApi = {
  login: (data: any) =>
    apiRequest<{ token: string }>("post", "/api/admin/login", data),
  verifyAdmin: (data: any) =>
    apiRequest("post", "/api/admin/verify-admin", data),
  ...
  resetPassword: (data: any) =>
    apiRequest("post", "/api/admin/reset-password", data),
...
const userApi = {
  ...
  loginUser: (data: any) =>
    apiRequest<{ token: string }>("post", "/api/users/login", data),
...
const paymentApi = {
  ...
  getRequestById: (requestId: string) =>
    apiRequest("get", `/api/payment/requests/${requestId}`),
  approveRequest: (requestId: string) =>
    apiRequest("post", `/api/payment/requests/${requestId}/approve`),
  rejectRequest: (requestId: string, comment: string) =>
    apiRequest("post", `/api/payment/requests/${requestId}/reject`, {
      comment,
    }),
  raiseRequests: ({ amount, remarks }) =>
    apiRequest<any[]>("post", "/api/payment/requests", { amount, remarks }),
};
...
const transactionApi = {
  ...
  getTransactionById: (transactionId: string) =>
    apiRequest("get", `/api/transactions/${transactionId}`),
  executeTransaction: (transactionId: string) =>
    apiRequest("post", `/api/transactions/${transactionId}/execute`),
  auditTransaction: (transactionId: string) =>
    apiRequest("post", `/api/transactions/${transactionId}/audit`),
};
...
const supportApi = {
  ...
  updateTicketStatus: (ticketId: string, data: any) =>
    apiRequest("put", `/api/support/tickets/${ticketId}/status`, data),
